...
impl RealtimeService {
    ...
    pub async fn new(mongodb_url: &str, database: &str) -> Result<Self> {
        ...
        Ok(Self { db, collection, snapshots })
    }
    ...
    pub async fn consolidate_all(&self) -> Result<usize> {
        ...
        let mut cursor = self.db.collection::<bson::Document>("rate_snapshots")
            .aggregate(pipeline)
            .await?;
        ...
        while let Some(Ok(doc_result)) = cursor.next().await {
            ...
            let latest_liquidity_usd = doc_result.get_i64("latest_liquidity_usd").unwrap_or(0) as u64;
            let latest_tvl_usd = doc_result.get_i64("latest_tvl_usd").unwrap_or(0) as u64;
            let latest_utilization_rate = doc_result.get_i32("latest_utilization_rate").unwrap_or(0) as u32;
            let latest_date = doc_result.get_datetime("latest_date")
                .ok()
                .map(|d| DateTime::from_timestamp_millis(d.timestamp_millis()).unwrap_or(now))
                .unwrap_or(now);
            let first_seen = doc_result.get_datetime("first_seen")
                .ok()
                .map(|d| DateTime::from_timestamp_millis(d.timestamp_millis()).unwrap_or(now))
                .unwrap_or(now);
            ...
            let vault_name = doc_result.get_str("latest_vault_name").ok().map(|s| s.to_string());
            let url = doc_result.get_str("latest_url").unwrap_or("").to_string();
            let operation_type_str = doc_result.get_str("latest_operation_type").unwrap_or("Lending");
            let action_str = doc_result.get_str("latest_action").unwrap_or("Supply");

            let protocol: crate::models::Protocol = serde_json::from_value(
                serde_json::Value::String(protocol_str.to_string())
            ).unwrap_or(crate::models::Protocol::Aave);
            let chain: crate::models::Chain = serde_json::from_value(
                serde_json::Value::String(chain_str.to_string())
            ).unwrap_or(crate::models::Chain::Ethereum);
            let operation_type: crate::models::OperationType = serde_json::from_value(
                serde_json::Value::String(operation_type_str.to_string())
            ).unwrap_or(crate::models::OperationType::Lending);
            let action: crate::models::Action = serde_json::from_value(
            ...
            let all_apys: Vec<f64> = doc_result.get_array("all_net_apys")
                .map(|arr| arr.iter().filter_map(|v| v.as_f64()).collect())
                .unwrap_or_default();
            let all_dates: Vec<bson::DateTime> = doc_result.get_array("all_dates")
                .map(|arr| arr.iter().filter_map(|v| v.as_datetime().cloned()).collect())
                .unwrap_or_default();

            // Build RateSnapshot-like structs for the existing time-weighted calculation
            let snapshots_for_calc: Vec<RateSnapshot> = all_apys.iter().zip(all_dates.iter())
            ...
            let pre_90d_fallback: Option<RateSnapshot> = all_apys.iter().zip(all_dates.iter())
                .filter(|(_, d)| d.timestamp_millis() < start_90d_millis)
            ...
            let asset_category = asset_obj.category()
                .first()
                .cloned()
            ...
            upsert_futures.push(async move {
                ...
                let mut options = mongodb::options::ReplaceOptions::default();
                options.upsert = Some(true);
                collection.replace_one(filter, &realtime_rate)
                    .with_options(options)
                    .await
            });
        }
        ...
        tracing::info!("Pipeline aggregation done, upserting {} vaults into rate_realtime...", total);

        let stream = futures::stream::iter(upsert_futures)
            .buffer_unordered(20);
        ...
        tracing::info!("Consolidated {} vaults successfully (pipeline)", consolidated_count);
    ...
    pub async fn query_rates(&self, query: &crate::models::RateQuery) -> Result<(Vec<crate::models::RateResult>, u64)> {
    ...
    pub async fn query_all_rates(&self, query: &crate::models::RateQuery) -> Result<Vec<crate::models::RateResult>> {
        let filter = Self::build_rate_filter(query);
    ...
    fn build_rate_filter(query: &crate::models::RateQuery) -> mongodb::bson::Document {
        ...
        if let Some(protocols) = query.parse_protocols() {
            let p_strings: Vec<String> = protocols.iter()
                .map(|p| serde_json::to_value(p).unwrap_or_default().as_str().unwrap_or("").to_string())
                .collect();
            filter.insert("protocol", doc! { "$in": &p_strings });
        }

        if let Some(chains) = query.parse_chains() {
            let c_strings: Vec<String> = chains.iter()
        ...
        if let Some(ref action) = query.action {
            let a_str = serde_json::to_value(action).unwrap_or_default().as_str().unwrap_or("").to_string();
            filter.insert("action", &a_str);
        }

        if query.min_liquidity > 0 {
            filter.insert("current.liquidity_usd", doc! { "$gte": query.min_liquidity as i64 });
        }
        ...
        if let Some(op_types) = query.parse_operation_types() {
            let op_strings: Vec<String> = op_types.iter()
                .map(|o| serde_json::to_value(o).unwrap_or_default().as_str().unwrap_or("").to_string())
                .collect();
