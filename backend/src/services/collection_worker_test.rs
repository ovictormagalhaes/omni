...
fn test_backfill_days_configuration() {
    ...
    assert!(
        backfill_days <= 365,
        "Backfill days should not exceed 1 year"
    );
}
...
fn test_vault_id_consistency() {
    use crate::models::{Action, Chain, OperationType, Protocol, RateSnapshot};
    ...
    assert_eq!(
        vault_id1, vault_id2,
        "Same parameters should generate same vault_id"
    );
    assert_eq!(
        vault_id1.len(),
        16,
        "Vault ID should be 16 characters (8 bytes hex)"
    );
    assert_ne!(
        vault_id1, vault_id_borrow,
...
fn test_collection_metrics_calculation() {
    ...
    assert_eq!(
        total_snapshots, 10,
        "Total snapshots should match operations"
    );
}
