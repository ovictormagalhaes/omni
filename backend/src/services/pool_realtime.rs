...
use crate::models::{
    RealtimePool, PoolSnapshot, PoolResult, PoolQuery, CurrentPoolData, PoolAprMetrics,
    AssetCategory, Asset, KnownAsset,
};
...
pub fn tokens_for_categories(categories: &[AssetCategory]) -> Vec<String> {
    ...
    let all_known = vec![
        ...
        SOL, STSOL, MSOL, JITOSOL, JUPSOL,
        TRX, LINK, AAVE, UNI, CRV, BAL, COMP,
    ];
...
impl PoolRealtimeService {
    ...
    async fn create_indexes(collection: &Collection<RealtimePool>) -> Result<()> {
        ...
        collection.create_indexes(vec![index_vault, index_pair, index_protocol]).await?;
    ...
    fn build_pool_filter(query: &PoolQuery) -> mongodb::bson::Document {
        ...
        if let Some(chains) = query.parse_chains() {
            let chain_strings: Vec<String> = chains.iter()
        ...
        if let Some(ref pt) = query.parse_pool_type() {
            let pt_str = serde_json::to_value(pt).unwrap_or_default().as_str().unwrap_or("").to_string();
        ...
        if query.min_volume > 0 {
            filter.insert("current.volume_24h_usd", doc! { "$gte": query.min_volume as f64 });
        }
        ...
        let tokens0 = cats0.as_ref().map(|c| tokens_for_categories(c)).unwrap_or_default();
        let tokens1 = cats1.as_ref().map(|c| tokens_for_categories(c)).unwrap_or_default();
    ...
    pub async fn consolidate_all(&self) -> Result<usize> {
        ...
        let db = &self.db;
        let mut cursor = db.collection::<mongodb::bson::Document>("pool_snapshots")
            .aggregate(pipeline)
            .await?;

        // Also get total snapshot counts per pool (including older than 30d)
        let count_pipeline = vec![
            doc! { "$group": { "_id": "$pool_vault_id", "total": { "$sum": 1 } } },
        ];
        let mut count_cursor = db.collection::<mongodb::bson::Document>("pool_snapshots")
            .aggregate(count_pipeline)
            .await?;

        let mut total_counts: std::collections::HashMap<String, i32> = std::collections::HashMap::new();
        ...
        let seven_days_ago_bson = mongodb::bson::DateTime::from_millis(seven_days_ago.timestamp_millis());
        ...
        while let Some(Ok(agg_doc)) = cursor.next().await {
            ...
            let all_aprs: Vec<f64> = agg_doc.get_array("all_aprs")
                .map(|arr| arr.iter().filter_map(|v| v.as_f64()).collect())
                .unwrap_or_default();
            let all_dates: Vec<mongodb::bson::DateTime> = agg_doc.get_array("all_dates")
                .map(|arr| arr.iter().filter_map(|v| v.as_datetime().cloned()).collect())
                .unwrap_or_default();

            let apr_7d_values: Vec<f64> = all_aprs.iter().zip(all_dates.iter())
            ...
            let volatility = if all_aprs.len() > 1 {
                let mean = apr_30d;
                let variance = all_aprs.iter()
                    .map(|apr| (apr - mean).powi(2))
                    .sum::<f64>() / (all_aprs.len() - 1) as f64;
                variance.sqrt()
            } else {
                0.0
            };

            let snapshot_count = total_counts.get(&pool_id).copied().unwrap_or(all_aprs.len() as i32);
        ...
        tracing::info!("Pool consolidation: upserting {} pools with metrics...", total);

        let collection = self.collection.clone();
        let update_futures: Vec<_> = updates.into_iter().map(|(pool_id, update)| {
            let coll = collection.clone();
            async move {
                coll.update_one(doc! { "pool_vault_id": &pool_id }, update).await
            }
        }).collect();
        ...
        tracing::info!("Consolidated {} pools into pool_realtime (aggregation pipeline)", consolidated);
