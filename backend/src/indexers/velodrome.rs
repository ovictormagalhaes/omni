...
// Velodrome is the primary DEX on Optimism (Solidly ve(3,3) fork).
// Subgraph uses Solidly-style schema: pairs, pairDayDatas, gauges.
// ============================================================================

const SUBGRAPH_ID: &str = "BsBDqDf6rJJyxKACZrCHAa8Gaf384cmL2hxfLaDuB8XM";

#[derive(Clone)]
pub struct VelodromeIndexer {
    client: reqwest::Client,
    graph_api_key: Option<String>,
}

// GraphQL response structures — identical to Aerodrome (same codebase)
...
impl VelodromeIndexer {
    ...
    pub async fn fetch_pools(&self) -> Result<Vec<PoolRate>> {
        let api_key = match &self.graph_api_key {
            Some(key) => key.clone(),
            None => {
                tracing::warn!("[Velodrome] No THE_GRAPH_API_KEY configured, skipping");
                return Ok(vec![]);
            }
        };
    ...
    async fn try_fetch_solidly(&self, url: &str) -> Result<Vec<PoolRate>> {
        ...
        tracing::info!("[Velodrome] Fetching pairs (Solidly schema) from subgraph");
    ...
    async fn try_fetch_uni_style(&self, url: &str) -> Result<Vec<PoolRate>> {
        ...
        tracing::info!("[Velodrome] Fetching pools (Uni-style schema) from subgraph");
    ...
    async fn execute_query(
        ...
        let data = match response.data {
            Some(d) => d,
            None => {
                tracing::warn!("[Velodrome] No data returned, schema may not match");
                return Ok(vec![]);
            }
        };
        ...
        tracing::info!("[Velodrome] Fetched {} items from subgraph", items.len());
        ...
        tracing::info!("[Velodrome] Parsed {} pools after filtering", rates.len());
        Ok(rates)
    }

    fn parse_pair(pair: SubgraphPair, is_solidly: bool) -> Option<PoolRate> {
        let token0 = Asset::from_symbol(&pair.token0.symbol, "Velodrome");
        ...
        Some(PoolRate {
            ...
            fee_apr_24h,
            fee_apr_7d,
            rewards_apr: 0.0, // TODO: Calculate from gauge emissions when VELO price available
    ...
    pub fn get_pool_url(pool_address: &str) -> String {
        format!("https://velodrome.finance/pool/{}", pool_address)
    }
}

#[async_trait]
impl PoolIndexer for VelodromeIndexer {
