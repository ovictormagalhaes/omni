...
mod tests {
            ...
            Ok(rates) => {
                ...
                if !rates.is_empty() {
                    ...
                    for rate in &rates {
                        ...
                        // Validate liquidity
                        assert!(rate.total_liquidity > 0, "Total liquidity should be positive");
                        ...
                        assert!(rate.utilization_rate <= 100.0,
                            "Utilization rate should be <= 100%");
                    }
                    
                    println!("✅ All {} Morpho rates have valid structure", rates.len());
                } else {
                    println!("⚠️  Morpho API returned no rates (possibly no markets match criteria)");
                }
            }
            Err(e) => {
    ...
    fn test_morpho_supported_chains() {
        ...
        for chain in supported {
            ...
            assert!(url.contains("morpho.org"), "URL should contain morpho.org for {:?}", chain);
            assert!(url.contains("/earn"), "URL should contain /earn for {:?}", chain);
        }
    ...
    async fn test_morpho_error_handling() {
        ...
        let indexer = MorphoIndexer::new("https://invalid-morpho-api.example.com/graphql".to_string());
