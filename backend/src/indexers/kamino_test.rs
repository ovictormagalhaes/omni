...
mod tests {
    ...
    #[test]
    fn test_parse_strategies_with_valid_apy() {
        let indexer = KaminoIndexer::new("https://api.kamino.finance".to_string());

        let strategies = vec![StrategyInfo {
        ...
        let vault = &rates[0];
        assert_eq!(vault.protocol, Protocol::Kamino);
    ...
    #[test]
    fn test_parse_strategies_skips_none_apy() {
        let indexer = KaminoIndexer::new("https://api.kamino.finance".to_string());

        let strategies = vec![StrategyInfo {
            strategy_name: "Inactive Vault".to_string(),
            pub_key: "vault_pubkey_2".to_string(),
            token_a_mint: "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v".to_string(),
            token_b_mint: "So11111111111111111111111111111111111111112".to_string(),
            apy: None,
            tvl_usd: Some(100.0),
        }];
    ...
    #[test]
    fn test_parse_strategies_unknown_token_a_falls_back_to_b() {
        let indexer = KaminoIndexer::new("https://api.kamino.finance".to_string());

        let strategies = vec![StrategyInfo {
            strategy_name: "Mystery-SOL Vault".to_string(),
            pub_key: "vault_pubkey_3".to_string(),
            token_a_mint: "UnknownMintAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA".to_string(),
            token_b_mint: "So11111111111111111111111111111111111111112".to_string(), // SOL
            apy: Some(0.07),
        ...
        let rates = indexer.parse_strategies(strategies);
        assert_eq!(rates.len(), 1);
        assert_eq!(rates[0].asset, Asset::Known(KnownAsset::SOL));
    }
