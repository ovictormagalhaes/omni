...
mod tests {
    ...
    fn test_get_protocol_url() {
        ...
        assert_eq!(
            indexer.get_protocol_url(&Chain::Solana, None),
            ""
        );
    }
    ...
    async fn test_aave_fetch_rates_arbitrum() {
        ...
        for rate in &rates {
            ...
            assert!(rate.total_liquidity > 0, 
                "Total liquidity should be positive, got: {}", rate.total_liquidity);
            ...
            assert!(rate.ltv >= 0.0 && rate.ltv <= 1.0,
                "Invalid LTV: {}", rate.ltv);
        }
        
        // Check for expected assets
        let assets: Vec<Asset> = rates.iter().map(|r| r.asset.clone()).collect();
        let has_usdc = assets.iter().any(|a| matches!(a, Asset::Known(KnownAsset::USDC)));
        let has_eth = assets.iter().any(|a| matches!(a, Asset::Known(KnownAsset::ETH)));
        
        assert!(has_usdc || has_eth, "Should have at least USDC or ETH in results");
    ...
    async fn test_aave_fetch_rates_base() {
        ...
        for rate in &rates {
            ...
            assert!(rate.total_liquidity > 0, 
                "Total liquidity missing for {}", rate.asset);
        }
        
        println!("✅ Aave Base test passed with {} rates", rates.len());
        for rate in &rates {
            println!("  {} - Supply: {:.2}% | Liquidity: ${}", 
    ...
    async fn test_aave_liquidity_calculation() {
        ...
        for rate in &rates {
            ...
            let expected_available = (rate.total_liquidity as f64 * (1.0 - utilization_decimal)).round() as u64;
            
            // Allow small rounding differences (within 1%)
            let diff_percent = if expected_available > 0 {
                ((rate.available_liquidity as f64 - expected_available as f64).abs() / expected_available as f64) * 100.0
            } else {
                0.0
            };
