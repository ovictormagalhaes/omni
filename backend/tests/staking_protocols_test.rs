use omni_backend::{
    indexers::{JitoIndexer, LidoIndexer, MarinadeIndexer, RocketPoolIndexer},
...
async fn test_all_staking_protocols() {
    ...
    assert_eq!(
        lido_eth.len(),
        2,
        "Lido should return 2 products on Ethereum (stETH + wstETH)"
    );
    ...
    assert_eq!(
        lido_sol.len(),
        1,
        "Lido should return 1 product on Solana (stSOL)"
    );
    ...
    assert_eq!(
        marinade_rates.len(),
        1,
        "Marinade should return 1 product (mSOL)"
    );
    ...
    assert_eq!(
        jito_rates.len(),
        1,
        "Jito should return 1 product (JitoSOL)"
    );
    ...
    assert_eq!(
        rp_rates.len(),
        1,
        "Rocket Pool should return 1 product (rETH)"
    );
...
async fn test_staking_protocols_wrong_chain() {
    ...
    assert_eq!(
        wrong_chain.len(),
        0,
        "Marinade should return 0 results on Ethereum"
    );
    ...
    assert_eq!(
        wrong_chain2.len(),
        0,
        "Rocket Pool should return 0 results on Solana"
    );
}
...
async fn test_staking_apy_ranges() {
    ...
    for rate in lido_eth {
        assert!(
            rate.supply_apy > 0.0 && rate.supply_apy < 20.0,
            "ETH staking APY should be 0-20%"
        );
    }
    ...
    for rate in marinade_rates {
        assert!(
            rate.supply_apy > 0.0 && rate.supply_apy < 15.0,
            "SOL staking APY should be 0-15%"
        );
    }
}
