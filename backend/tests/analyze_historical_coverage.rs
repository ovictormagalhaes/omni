...
async fn analyze_vaults_with_historical_data() -> Result<()> {
    ...
    let mut protocol_stats: std::collections::HashMap<String, usize> =
    ...
    while let Some(doc) = cursor.try_next().await? {
        ...
        let protocol = id
            .and_then(|d: &mongodb::bson::Document| d.get_str("protocol").ok())
            .unwrap_or("N/A");
        let chain = id
            .and_then(|d: &mongodb::bson::Document| d.get_str("chain").ok())
            .unwrap_or("N/A");
        let asset = id
            .and_then(|d: &mongodb::bson::Document| d.get_str("asset").ok())
            .unwrap_or("N/A");
        let operation = id
            .and_then(|d: &mongodb::bson::Document| d.get_str("operation_type").ok())
            .unwrap_or("N/A");
    ...
    println!(
        "Average snapshots per vault: {:.1}",
        total_snapshots as f64 / vaults_with_history as f64
    );
    ...
    println!(
        "\nTotal vaults with only current snapshot: {}",
        count_single
    );
