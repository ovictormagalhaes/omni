...
async fn test_daily_history_aggregation() -> Result<()> {
    ...
    assert_eq!(
        result.points.len(),
        90,
        "Should always return 90 daily points"
    );
    ...
    println!(
        "✅ All {} points are exactly 1 day apart",
        result.points.len()
    );
    ...
    println!(
        "   Days with APY changes: {}",
        result.points.len() - same_value_count - 1
    );
    ...
    println!(
        "✅ Received {} points (aggregated from 655 events)",
        result2.points.len()
    );
    assert_eq!(
        result2.points.len(),
        90,
        "Should aggregate multiple events per day into 90 points"
    );
    ...
    let result3 = historical_service
        .get_vault_history(None, Some(&Protocol::Morpho), None, None)
        .await?;
