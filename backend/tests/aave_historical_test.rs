...
async fn test_aave_base_usdc_has_historical_data() -> Result<()> {
    ...
    let api_key = env::var("THE_GRAPH_API_KEY").expect("THE_GRAPH_API_KEY must be set");
    ...
    if let Some(data) = response_data.data {
        ...
        println!(
            "\n✅ SUCCESS: Found {} historical data points",
            points.len()
        );
        ...
        assert!(
            points.len() > 1,
            "Expected more than 1 historical point, got {}",
            points.len()
        );
        ...
        println!(
            "  Points per day (avg): {:.1}",
            points.len() as f64 / unique_days.len() as f64
        );

        // This proves the subgraph HAS historical data
        assert!(
            unique_days.len() >= 20,
            "Expected at least 20 unique days of data, got {}",
            unique_days.len()
        );
        ...
        println!(
            "✅ CONFIRMED: Total of {} data points need deduplication",
            points.len()
        );
    } else {
